...
import 'package:TennisApp/newMatch/newMatchLastPage.dart';
import 'package:TennisApp/newMatch/thePoint/Rally.dart';
...
class MatchPanel extends StatefulWidget {
  ...
  final List<List<int>> gameScorePackage; //1:st = gameScore,  2:nd = first set score 3:rd = second set score .....
final int whoServes;
  @override
...
Matches1 opponenttTournamnet = widget.opponentstournamentDataPackLiveStats.matches[0];
    ...
    firstServeprocent = ((yourtournament.firstServeProcentage ) / (yourtournament.servePointsPlayed - yourtournament.aces + 1 )) * 100;
    secondServeprocent = ((yourtournament.secondServeProcentage - 1 ) / (yourtournament.servePointsPlayed - yourtournament.firstServeProcentage - yourtournament.aces +2 ))  * 100;
 opponentsecondServeprocent = (opponenttTournamnet.secondServeProcentage - 1 ) / (yourtournament.recevingPointsPlayed - opponenttTournamnet.firstServeProcentage +1)  * 100;
opponentfirstServeprocent = ((yourtournament.firstServeProcentage ) / (yourtournament.servePointsPlayed - yourtournament.aces + 1 )) * 100;
 ...
 reference = databaseReference.child(url + key + "/" + "playerTournaments" + "/" + widget.tournamentDataPack.tournamentName + "/" + "match" + " " + widget.tournamentDataPack.matches[0].matchNumber.toString()).push();
...
timebased = true;
        ...
        "yourStats": [yourtournament.aces, yourtournament.doubleFaults, firstServeprocent, yourtournament.forcedErrors, yourtournament.pointsLost,yourtournament.pointsPlayed, yourtournament.pointsWon, yourtournament.returnErrors, yourtournament.returnWinner,secondServeprocent, yourtournament.unforcedErrors, yourtournament.voleyErrors, yourtournament.voleyWinner, yourtournament.winners, 0.22], // ABC: Alphabetisk ordning
        "opponentStats": [opponenttTournamnet.aces, opponenttTournamnet.doubleFaults, opponentfirstServeprocent, opponenttTournamnet.forcedErrors, opponenttTournamnet.pointsLost,opponenttTournamnet.pointsPlayed, opponenttTournamnet.pointsWon, opponenttTournamnet.returnErrors, opponenttTournamnet.returnWinner, opponentsecondServeprocent, opponenttTournamnet.unforcedErrors, opponenttTournamnet.voleyErrors, opponenttTournamnet.voleyWinner, opponenttTournamnet.winners,], // ABC: Alphabetisk ordning
        "trackedStats": [ace, doubleFault, firstServe, forcedErrors, returnError, returnWinner, secondServe, unforcedErrors, voleyError,voleyWinner, winners],
        ...
        "mostgameswinsformat": yourtournament.rules.matchFormatVariable.mostGamesWinsFormat, 
        "surface": yourtournament.surface,
...
DatabaseReference reference = databaseReference.child("LiveResults/"+ widget.matchID + "/");
reference.remove();
reference.push();
Matches yourtournament = widget.yourtournamentDataPackLiveStats.matches[0];
Matches1 opponenttTournamnet = widget.opponentstournamentDataPackLiveStats.matches[0];
...
double opponentfirstServeprocent;
double opponentsecondServeprocent;
print("serverpointsplayed "+ yourtournament.servePointsPlayed.toString() );
if(castMatchPressed){
...
opponentfirstServeprocent = (opponenttTournamnet.firstServeProcentage - 1) / (yourtournament.recevingPointsPlayed) * 100;
} else {
...
opponentsecondServeprocent = (opponenttTournamnet.secondServeProcentage - 1 ) / (yourtournament.recevingPointsPlayed - opponenttTournamnet.firstServeProcentage +1)  * 100;
  ...
  opponentfirstServeprocent = opponenttTournamnet.firstServeProcentage.toDouble(); 
  opponentsecondServeprocent = opponenttTournamnet.secondServeProcentage.toDouble();
}
...
Map<String, dynamic> accountdata = {
  ...
  void setScore(int whoWon, bool serverWon) {
    ...
    if(widget.yourtournamentDataPackLiveStats.matches[0].pointsPlayed == null){
...
widget.yourtournamentDataPackLiveStats.matches[0].recevingPointsPlayed = 0;
...
widget.yourtournamentDataPackLiveStats.matches[0].pointsLost = 0;
    ...
    MatchFormat scoreUrl =
      ...
      if(widget.whoServes == 1){
        widget.yourtournamentDataPackLiveStats.matches[0].servePointsPlayed++;
    ...
    if (scoreUrl.mostSetsWinsFormat) {
      if (scoreUrl.gamesPerSet > firstsetStandings[0] &&
          ...
          scoreUrl.gamesPerSet == firstsetStandings[0] &&
              scoreUrl.gamesPerSet == firstsetStandings[1] || scoreUrl.gamesPerSet == 6 &&
          ...
          if (scoreUrl.gamesPerSet > thirdsetStandings[0] &&
              scoreUrl.gamesPerSet > thirdsetStandings[1]) {
                 ...
                 if(scoreUrl.numberSets >= 3){
                    ...
                    if(secondsetStandings[0] > secondsetStandings[1] && firstsetStandings[0] < firstsetStandings[1]||secondsetStandings[0] < secondsetStandings[1] && firstsetStandings[0] > firstsetStandings[1] ){
...
setState(() {
    ...
    scorePack = [gameStandings, firstsetStandings, secondsetStandings, thirdsetStandings, fourthsetStandings, fifthsetStandings];
...
if (widget.whoServes == 1) {
    ...
    print("whoservearg = "+ whoservesarg.toString());
                            if (trackStats) {
                              if (firstServe) {
                                if (whoservesarg == 1 || widget.whoServes == 1) {
                                  Navigator.push(
                                      context,
                                      MaterialPageRoute(
                                          builder: (_) => Serve(
