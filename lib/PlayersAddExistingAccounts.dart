...
import 'package:TennisApp/UnusedStuff/Colors.dart';
import 'package:TennisApp/UnusedStuff/ParentCoachMainPage.dart';
import 'package:TennisApp/UnusedStuff/TennisPlayerHomePage.dart';
import 'package:TennisApp/WelcomePage.dart';
...
class _AddExistingState extends State<AddExisting> {
  ...
  bool result = false;
    ...
    getAllAccounts(path, name, password).then((value) => {
          ...
          this.setState(() {
            ...
            if(ter == 1) {
            if(path == "asdads") {
              this.setState(() { Navigator.push(context,
                        MaterialPageRoute(builder: (_) => CPHomePage()));});} else {
                          this.setState(() {
 ...
 Map<String, dynamic> accountdata = {
    ...
    if (t[1] == 1) {
      ...
      this.setState(() {
        _build = Row(children: [Text("")],);
        });
      });
    } else {this.setState(() {
        _build = errorMessage("No such email is connected to an account");
      });
      _timer = new Timer(const Duration(milliseconds: 3000), () {
        this.setState(() {
        _build = Row(children: [Text(""),]);
  ...
  Widget _build = Row(children: [Text("")],);
  Widget errorMessage(String text) {
    return Row(children: [Text(text, style: TextStyle(color: Colors.red, fontSize: 14, ), textAlign: TextAlign.center,)], mainAxisAlignment: MainAxisAlignment.center,);
  }
 ...
 _buildFooterLogo() {
  ...
  _buildTextField(
      TextEditingController controller, IconData icon, String labelText, bool obscure) {
    return Container(
